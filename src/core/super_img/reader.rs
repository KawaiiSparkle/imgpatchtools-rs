...
pub fn read_metadata(path: &Path, slot: u32) -> Result<LpMetadata> {
    ...
    let slot_off = md_start + (slot as usize) * (geo.metadata_max_size as usize);
        ...
        slot_off + geo.metadata_max_size as usize <= data.len(),
        "slot {} beyond file",
        slot
    );

    parse_metadata_blob(
        &data[slot_off..slot_off + geo.metadata_max_size as usize],
        geo,
    )
}
...
fn parse_metadata_blob(blob: &[u8], geometry: LpMetadataGeometry) -> Result<LpMetadata> {
    ...
    let minor = r16(blob, 6);
        ...
        major == LP_METADATA_MAJOR_VERSION,
...
fn parse_td(b: &[u8], o: usize) -> LpMetadataTableDescriptor {
    LpMetadataTableDescriptor {
        offset: r32(b, o),
        num_entries: r32(b, o + 4),
        entry_size: r32(b, o + 8),
    }
}
fn parse_part(b: &[u8]) -> LpMetadataPartition {
    ...
    LpMetadataPartition {
        name,
        attributes: r32(b, 36),
        first_extent_index: r32(b, 40),
        num_extents: r32(b, 44),
        group_index: r32(b, 48),
    }
}
...
fn parse_bd(b: &[u8]) -> LpMetadataBlockDevice {
    ...
    LpMetadataBlockDevice {
        first_logical_sector: r64(b, 0),
        alignment: r32(b, 8),
        alignment_offset: r32(b, 12),
        size: r64(b, 16),
        partition_name: name,
        flags: r32(b, 60),
    }

...
impl TransferList {
    ...
    pub fn total_blocks(&self) -> u64 { self.header.total_blocks }
}
...
pub fn parse_transfer_list(content: &str) -> Result<TransferList> {
    ...
    for (idx, &line) in lines[cmd_start..].iter().enumerate() {
        let cmd = parse_command(line, header.version)
            .with_context(|| format!("line {} (command #{}): {line:?}", cmd_start + idx + 1, idx))?;
        commands.push(cmd);
    }
...
fn parse_header(lines: &[&str]) -> Result<TransferListHeader> {
    ...
    let version: u32 = lines[0].parse().with_context(|| format!("bad version: {:?}", lines[0]))?;
    ensure!(
    ...
    let (stash_max_entries, stash_max_blocks) = if version >= 2 {
        ensure!(lines.len() >= 4, "v{version} transfer list missing stash header lines");
        let e: u32 = lines[2].parse().with_context(|| format!("bad stash_max_entries: {:?}", lines[2]))?;
        let b: u32 = lines[3].parse().with_context(|| format!("bad stash_max_blocks: {:?}", lines[3]))?;
        (e, b)
    } else {
        (0, 0)
    };

    Ok(TransferListHeader { version, total_blocks, stash_max_entries, stash_max_blocks })
}
...
fn parse_command(line: &str, version: u32) -> Result<TransferCommand> {
    ...
    match cmd_type {
        CommandType::Zero | CommandType::New | CommandType::Erase => parse_simple_cmd(cmd_type, &tokens, &mut pos, line),
        ...
        CommandType::Bsdiff | CommandType::Imgdiff => parse_patch_cmd(cmd_type, &tokens, &mut pos, line, version),
    }
}

fn parse_simple_cmd(cmd_type: CommandType, tokens: &[&str], pos: &mut usize, line: &str) -> Result<TransferCommand> {
...
fn parse_stash_cmd(tokens: &[&str], pos: &mut usize, line: &str, version: u32) -> Result<TransferCommand> {
...
fn parse_free_cmd(tokens: &[&str], pos: &mut usize, line: &str, version: u32) -> Result<TransferCommand> {
...
fn parse_move_cmd(tokens: &[&str], pos: &mut usize, line: &str, version: u32) -> Result<TransferCommand> {
...
fn parse_patch_cmd(cmd_type: CommandType, tokens: &[&str], pos: &mut usize, line: &str, version: u32) -> Result<TransferCommand> {
...
fn parse_source_spec(
    ...
    pos: &mut usize,
    version: u32,
) -> Result<(Option<u64>, Option<RangeSet>, Option<RangeSet>, Vec<(String, RangeSet)>)> {
    if version == 1 {
        let src = read_rangeset(tokens, pos, "src_ranges")?;
    ...
    while *pos < tokens.len() {
        ...
        *pos += 1;
        ...
        if tok.contains(':') {
            ...
            let rs = RangeSet::parse(tok).with_context(|| format!("bad buffer map rangeset: {tok:?}"))?;
...
fn parse_stash_ref(token: &str) -> Result<(String, RangeSet)> {
    ...
    ensure!(!range_str.is_empty(), "empty range set in stash ref: {token:?}");

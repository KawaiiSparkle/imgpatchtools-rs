...
impl ZeroCopyBzDecoder {
    ...
    fn new(compressed: &[u8]) -> Result<Self> {
        ...
        let ret = unsafe { BZ2_bzDecompressInit(&mut stream, 0, 0) };

        ensure!(ret == BZ_OK, "BZ2_bzDecompressInit failed: {}", ret);
    ...
    fn decompress_exact(&mut self, output: &mut [u8]) -> Result<()> {
        ...
        while total_produced < target_len {
            ...
            match ret {
                ...
                n if n == BZ_STREAM_END => {
                    // 流正常结束
                    if total_produced < target_len {
                        bail!(
                            "Compressed stream ended early: got {}, expected {}",
                            total_produced,
                            target_len
                        );
                    }
            ...
            if src_len > 0 {
                unsafe {
                    ...
                    while i < bulk_end {
                        ...
                        *diff_ptr.add(i + 1) =
                            (*diff_ptr.add(i + 1)).wrapping_add(*src_ptr.add(i + 1));
                        *diff_ptr.add(i + 2) =
                            (*diff_ptr.add(i + 2)).wrapping_add(*src_ptr.add(i + 2));
                        *diff_ptr.add(i + 3) =
                            (*diff_ptr.add(i + 3)).wrapping_add(*src_ptr.add(i + 3));
                        *diff_ptr.add(i + 4) =
                            (*diff_ptr.add(i + 4)).wrapping_add(*src_ptr.add(i + 4));
                        *diff_ptr.add(i + 5) =
                            (*diff_ptr.add(i + 5)).wrapping_add(*src_ptr.add(i + 5));
                        *diff_ptr.add(i + 6) =
                            (*diff_ptr.add(i + 6)).wrapping_add(*src_ptr.add(i + 6));
                        *diff_ptr.add(i + 7) =
                            (*diff_ptr.add(i + 7)).wrapping_add(*src_ptr.add(i + 7));
                        i += 8;
                    }
                    // 剩余字节
                    while i < overlap {
...
pub fn apply_bspatch_zero_vec(source: &[u8], patch: &[u8], patch_offset: usize) -> Result<Vec<u8>> {
    // 解析头部获取输出大小
    ensure!(patch.len() >= patch_offset + HEADER_SIZE, "patch too short");
    let hdr = &patch[patch_offset..patch_offset + HEADER_SIZE];
    ensure!(&hdr[..8] == b"BSDIFF40", "bad bsdiff magic");

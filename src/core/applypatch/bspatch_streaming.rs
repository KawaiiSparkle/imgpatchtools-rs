...
impl<'a> MemorySource<'a> {
    ...
    ///
    /// # Safety
    /// `offset` 必须在 `0..self.len` 范围内，否则会导致未定义行为。
    #[inline(always)]
...
impl<'a> BlockFileSink<'a> {
    pub fn new(
        ...
        let first_offset = ranges
            .iter()
            .next()
            .map(|(start, _end)| start * block_size as u64)
            .unwrap_or(0);
        ...
        while !remaining.is_empty() {
            ...
            if self.current_range_offset >= range_bytes {
                ...
                if self.current_range_idx < ranges_vec.len() {
                    ...
                    let offset = next_start * self.block_size as u64;
                    self.file
                        .file_mut()
...
pub fn apply_bspatch_streaming<S: SourceProvider, D: DataSink>(
    ...
    while new_pos < new_size {
        ...
        while extra_remaining > 0 {
            ...
            extra_stream
                .read_exact(&mut extra_buf[..chunk])
                .context("read extra")?;
